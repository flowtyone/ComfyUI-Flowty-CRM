...
def generate3d(model, rgb, ccm, device):
    ...
    with torch.no_grad():
        ...
        data_config["verts"] = data_config["verts"].cpu().contiguous()
        data_config["faces"] = data_config["faces"].cpu().contiguous()

    mesh = model.export_mesh(data_config, tri_fea_2=triplane_feature2)

    return mesh


def generate3d_cuda(model, rgb, ccm, device):
    ...
    verts, faces = clean_mesh(data_config['verts'].squeeze().cpu().numpy().astype(np.float32),
                              data_config['faces'].squeeze().cpu().numpy().astype(np.int32), repair=False,
                              remesh=False, remesh_size=0.005)
    data_config['verts'] = torch.from_numpy(verts).cuda().contiguous()
    data_config['faces'] = torch.from_numpy(faces).cuda().contiguous()

    with torch.no_grad():

        mesh_path_obj = path.join(temp_directory, "tmp")
        ...
        # mesh_path_glb = path.join(temp_directory, "tmp")
